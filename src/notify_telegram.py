...
def build_topic_keyboard(item_id: str, selected_formats: list, rubric: str = None) -> dict:
    ...
    «Статья» скрыта целиком, если модуль статей выключен секретом
    ARTICLES_ENABLED (решение 22.08.2026 — статьи теперь отдельная включаемая
    линия контента, как EN/pains, а не всегда-доступный формат). Ограничение
    «только темы из банка болей» (было с 16.08.2026) снято 29.08.2026 —
    заказчик прямо попросил статьи и по темам из мониторинга («Wildberries»,
    «AI-внедрение» и т.п.), боль-темы показались ему слишком пресными;
    write_article.py/write-article.md уже умеют оба режима (source-grounded
    vs боль-экспертиза), просто параметр `rubric` этой функции с 29.08.2026
    больше не используется для решения — оставлен в сигнатуре ради
    совместимости вызова.
    ...
    statya_allowed = articles_enabled
    ru_formats = [
