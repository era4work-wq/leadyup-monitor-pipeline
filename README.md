...
3. **`write-drafts.yml`** — запускается вручную (Actions → Run workflow), пока
   без расписания: `write_draft.py` пишет черновик по каждой теме из
...
`write-drafts.yml` пока запускается только вручную — переводить на расписание
имеет смысл после того, как несколько черновиков подряд устроят по качеству.
